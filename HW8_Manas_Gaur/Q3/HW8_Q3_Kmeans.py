...
tweets = []
for line in open('project_tweets.txt').readlines():
...
# K-means clustering
# number of clusters in this program has been obtained from Elbow /Silhouette method
km = KMeans(n_clusters = 100, n_init = 100) # try 100 different initial centroids
km.fit(X)
...
for idx, cls in enumerate(km.labels_):
    if cluster_stat.has_key(cls):
        ...
        cluster_stat[cls] = 1
    open('/Users/manasgaur/Desktop/MyApp/Multiprocessing/HW8_Manas_Gaur/clusterQ4/cluster-{0}.txt'.format(cls), 'a').write(json.dumps(tweets[idx]) + '\r\n')
