...
for tweet_id, class_label, tweet_text in tweets:
    ...
    X.append(x)
print "performing the first part of the question that is printing the prediction file"
...
X = []
for tweet_id, tweets_text in tweets:
    x = [0] * len(vocab)
    terms = [term for term in tweets_text.split() if len(term) > 2]
    for term in terms:
        if vocab.has_key(term):
            x[vocab[term]] += 1
    X.append(x)
...
print " performing the second part of the question which is top K "
